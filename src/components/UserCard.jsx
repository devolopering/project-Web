    ...
    <div className='max-w-[1261px] mx-auto pb-[130px]'>
      <div className='grid grid-cols-1 md:grid-cols-2 mt-[50px] py-[6px] lg:grid-cols-3 gap-4'>
        {cardText.map((card, index) => (
          <div
            key={index}
            className='pl-8 pt-8 pb-10 rounded-md cursor-pointer transition-all duration-300 bg-[#161618] hover:bg-[#FD560B] hover:text-white' // Apply hover styles to parent div
          >
            <h3 className='text-[30px] font-bold font-sf-pro-display transition-colors duration-300'>
              {card.title}
            </h3>
            <p className='mt-8 font-sf-pro-display font-normal text-[19px] mr-16 text-[#A6A4A4] transition-colors duration-300'>
              {card.text}
            </p>
          </div>
        ))}
